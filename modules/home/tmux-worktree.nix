{
  ...
  cfg = config.myModules.home.tmux-worktree;
  ...
  # Script to create new window with worktree awareness
  tmux-new-window = pkgs.writeShellScriptBin "tmux-new-window" ''
    #!/usr/bin/env bash
  ...
  tmux-worktree = pkgs.writeShellScriptBin "tmux-worktree" ''
    # ============================================================================
  ...
  '';
in {
  options.myModules.home.tmux-worktree = {
    enable = lib.mkEnableOption "tmux-worktree for git worktree window management";
  };

  config = lib.mkIf cfg.enable {
    home.packages = lib.mkIf (!portable) [tmux-worktree tmux-new-window];

    home.file.".local/bin/tmux-worktree" = lib.mkIf portable {
      executable = true;
      text = builtins.readFile "${tmux-worktree}/bin/tmux-worktree";
    };

    home.file.".local/bin/tmux-new-window" = lib.mkIf portable {
      executable = true;
      text = builtins.readFile "${tmux-new-window}/bin/tmux-new-window";
    };
  };
}
